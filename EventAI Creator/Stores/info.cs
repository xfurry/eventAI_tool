...
/*43 | 01 */{"MOUNT_TO_ENTRY_OR_MODEL","CreatureEntry","ModelId","","Set mount model from creature_template.entry (Param1) OR explicit modelId (Param2). If (Param1) AND (Param2) are both 0, unmount."}
        ...
        {
            // command name         // datalong     // datalong         // details
/* 0 */     {"TALK",                "ChatType",     "language",         "Creature say/whisper/yell/textemote."},
/* 1 */     {"EMOTE",               "emote_id",     "",                 "Play emote on creature."},
/* 2 */     {"FIELD_SET",           "field_id",     "field value",      "Change the value at an index for the unit."},
/* 3 */     {"MOVE_TO",             "",             "travel time",      "Relocate creature to a destination"},
/* 4 */     {"FLAG_SET",            "field_id",     "bitmask",          "Turns on bits on a flag field at an index for the worldobject."},
/* 5 */     {"FLAG_REMOVE",         "field_id",     "bitmask",          "Turns off bits on a flag field at an index for the worldobject."},
/* 6 */     {"TELEPORT_TO",         "map_id",       "",                 "Teleports the player to a location."},
/* 7 */     {"QUEST_EXPLORED",      "quest_id",     "distance",         "Satisfies the explore requirement for a quest."},
/* 8 */     {"KILL_CREDIT",         "creature entry","bool",            "Satisfies the kill credit requirement for a quest."},
/* 9 */     {"RESPAWN_GAMEOBJECT",  "db_guid",      "despawn_delay",    "Spawns a despawned gameobject."},
/* 10 */    {"TEMP_SUMMON_CREATURE","creature entry","despawn_delay",   "Temporarily summon a creature."},
/* 11 */    {"OPEN_DOOR",           "db_guid",      "reset_delay",      "Opens a door gameobject (type == 0)."},
/* 12 */    {"CLOSE_DOOR",          "db_guid",      "reset_delay",      "Closes a door gameobject (type == 0)."},
/* 13 */    {"ACTIVATE_OBJECT",     "",             "",                 "Activates an object."},
/* 14 */    {"REMOVE_AURA",         "spell_id",     "",                 "Removes an aura due to a spell."},
/* 15 */    {"CAST_SPELL",          "spell_id",     "",                 "Casts a spell."},
/* 16 */    {"PLAY_SOUND",          "sound_id",     "bitmask",          "Plays a sound."},
/* 17 */    {"CREATE_ITEM",         "item entry",   "amount",           "Creates an item."},
/* 18 */    {"DESPAWN_SELF",        "despawn delay","",                 "Despawns an npc with some delay."},
/* 19 */    {"PLAY_MOVIE",          "movie id",     "",                 "Plays a movie."},
/* 20 */    {"MOVEMENT",            "MovementType", "",                 "Change Movement of a creature."},
/* 21 */    {"SET_ACTIVEOBJECT",    "bool",         "",                 "Sets the active object state of the creature"},
/* 22 */    {"SET_FACTION",         "factionId",    "TemporaryFactionFlags","Sets the faction id from the creature."},
/* 23 */    {"MORPH_TO_ENTRY_OR_MODEL","entry/modelid","",              "Morphs the creature to the model specified."},
/* 24 */    {"MOUNT_TO_ENTRY_OR_MODEL","entry/modelid","",              "Mounts the creature with the model specified."},
/* 25 */    {"SET_RUN",             "bool",         "",                 "Changes the run capacity of the creature."},
/* 26 */    {"ATTACK_START",        "",             "",                 "Makes a creature attack a unit"},
/* 27 */    {"GO_LOCK_STATE",       "Flag",         "",                 "Lock/Unlock a gameobject"},
/* 28 */    {"STAND_STATE",         "stand state",  "",                 "Set the stand state of a creature"},
/* 29 */    {"MODIFY_NPC_FLAGS",    "NPCFlags",     "bitmask",          "Modify the NPC flags of a creature"},
        };
    }
}
